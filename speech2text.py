...
from nemo.collections.asr.helpers import monitor_asr_train_progress, process_evaluation_batch, process_evaluation_epoch, word_error_rate
...
def main():
    ...
    #added TensorBoard logging here
    parser.add_argument("--tboard_exp_name", default=None, type=str)
    #parser.add_argument("--tboard_project", default=None, type=str)
    ...
    train_data_layer = nemo_asr.AudioToTextDataLayer(
        ...
        shuffle=True,
        normalize_transcripts=False,  #args.normalize_text,
    )
...
#    wer = word_error_rate(hypotheses = predictions, references = transcript)
    ...
    # Implementing TensorBoard logging
    if args.tboard_exp_name is not None:
        tb_writer = SummaryWriter(log_dir = 'runs/'+args.tboard_exp_name)
        #tb_writer.add_scalar('loss/wer', wer, 100)
        tboard_callback = nemo.core.TensorboardLogger(tb_writer)
